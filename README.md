...
b)	Do a walk-through of the end-to-end process as a recorded session for reference
c)	Team leader will only sign off on access to tooling (Python library, VS Code,etc) after you complete the above and demonstrate an understanding of the business context and risks
...
🟠**You own your product outcomes and are responsible for checking and verifying any changes, including code or updates created via use of AI**
If you’re unsure, ask someone before committing code. 

🟠A future engineer should begin by reviewing:

- `src/ingest.py`
- `src/transform.py`
- `logs/data_quality_log.csv`
- the Power BI semantic model and DAX measures
______________________________________
...
Data Sources
Source		        Type	        Purpose
Yahoo Finance	 	Market Index	S&P 500 daily market performance
FRED                Macro Series	VIX volatility index
...
This separation allows the platform to evolve independently over time without too tightly coupling operational pipelines to reporting outputs. ________________________________________
...
Both datasets were aligned to a shared reporting period beginning 1 April 2024 for demonstration purposes.  
🔴**This filter should be removed in a final production environment.**


2.	Trading Day Handling
...
TELEMETRY /
 OBSERVABILITY ITEM	                    HOW IT IS ADDRESSED IN THE CODE
...
________________________________________
Dashboard Design
The target audience is financially literate business users. 
🟢The dashboard answers the question: **“How has market activity trended over the past 90 days, and is there anything we should be watching?”**
It focuses on:
...
- The solution is locally executable and intentionally lightweight for the case study.
- Scheduling is demonstrated through repeatable scripts rather than production orchestration.
- Fabric Pipelines, Purview, and Ontology are discussed as future-state architecture, not implemented in this local version.
- RAG thresholds are explainable management thresholds, not statistically back-tested trading signals.

## Future Enhancements

- Operationalise ingestion and transformation through Microsoft Fabric Pipelines.
- Add pipeline telemetry such as run ID, duration, row counts, and freshness checks.
- Extend data quality monitoring with automated alerts.
- Add further market or macroeconomic indicators.
- Back-test RAG thresholds against historical stress periods.
- Explore Fabric Ontology and data agents for trusted semantic querying.



🟠Before making changes, they should confirm:
- source definitions
...
## AI Agent Use Summary

AI was used to accelerate drafting, coding, DAX iteration, documentation, and review. Outputs were not accepted uncritically. Key corrections included:
- correcting timezone handling and investigate relevance of UTC vs ET 
- preserving missing observations rather than silently dropping them
- moving trading-day logic into the curated/semantic model
